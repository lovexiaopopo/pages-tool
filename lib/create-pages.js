...
module.exports = class {
  ...
  loadFile(dirPath) {
    ...
    console.log('loadFile---dirItems---', dirItems)
    dirItems.forEach(item => {
      console.log('loadFile---dirItems----item', item)
      if (this.watchFileName.indexOf(item) > -1) {
        files.push(path.join(dirPath, item));
        console.log('loadfile----dirPath-if', dirPath, files)
      } else {
        if (fs.statSync(path.join(dirPath, item)).isDirectory()) {
          ...
          console.log('loadfile----dirPath-else', dirPath, files)
        }
      }
    });
    return files;
